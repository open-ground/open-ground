 ...
 * Tbase（人大金仓）方言实现
 *
 * <p>Tbase 兼容 PostgreSQL 语法，但元数据查询有细微差异。
 *
 * @author open-ground
 * @since 1.0.2
 */
@Component
public class TbaseDialect implements DbDialect {

    @Override
    public String getDbType() {
        return DbTypeDetector.GAUSSDB;
    }
    ...
    public String getTableListSql(String dbName) {
        ...
        return "SELECT DISTINCT t1.table_name AS TABLE_NAME, 'BASE TABLE' as TABLE_TYPE, "
                + "t2.description AS TABLE_COMMENT "
                + "FROM (SELECT table_name, oid FROM information_schema.tables a "
                + "LEFT JOIN (SELECT oid, relname FROM pg_class GROUP BY oid, relname) b "
    ...
    public String getTableColInfoSql(String dbName, String tableName) {
        ...
        return "SELECT A.attnum AS ORDINAL_POSITION, A.attname AS COLUMN_NAME, "
                ...
                + "LEFT JOIN (SELECT DISTINCT pg_attribute.attname FROM pg_index, pg_class, pg_attribute "
